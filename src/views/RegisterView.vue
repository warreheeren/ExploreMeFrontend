    ...
    <div class="w-full max-w-sm">
      <div class="bg-white border border-gray-200 rounded-xl p-8">
        <div class="text-center mb-8">
          ...
          <h1 class="text-xl font-bold text-gray-900">Account aanmaken</h1>
          <p class="text-sm text-gray-400 mt-1">Begin met delen op ExploreMe</p>
        </div>
      ...
      <p class="text-sm text-gray-400 text-center mt-5">
        Al een account?
        <RouterLink to="/login" class="text-primary-600 font-semibold hover:text-primary-700 hover:underline">Inloggen</RouterLink>
      </p>
    </div>
  </div>
</template>
...
async function onSubmit() {
  ...
  try {
    ...
    error.value = e?.response?.data?.title || e?.response?.data?.message || 'Registratie mislukt'
  } finally {
    loading.value = false
  }
}
